...
function init() {
    ...
    var solarSystem = { size     : 25.0,			
			children : [ { id : "mercury", label : "Mercury", size : 4480,   distance : 58000000,   perihelion : 115.88   },
				     ...
				     { id : "saturn",  label : "Saturn",  size : 129536, distance : 1433400000, perihelion : 378.09,   
				       children : [ { "id" : "saturn.ring", type : "ring", size : 129536, distance : 1433400000, perihelion : 378.09 } ]
				     },
    ...
    var	textures = { "mercury"     : { source : "img/mercurymap.jpg",    texture : null },
		     ...
		     "saturn.ring" : { source : "img/saturnringcolor.rot.jpg",     texture : null },
		     "saturn.ring.alpha" : { source : "img/saturnringpattern.rot.png",     texture : null }
		   };
    for( k in textures ) {
...
function init_withTextures( solarSystem, textures ) {
    ...
    this.renderer.setSize( window.innerWidth-2, 
			   window.innerHeight-4  // scrollbars :(
			 ); 
    ...
    var options        = { distanceFactor : 0.00000003,
			   sizeFactor     : 0.0001
			 };
    /*
    ...
    */
    ...
    buildSolarSystem( parentMesh, solarSystem, textures, origin, options );
    ...
    // Add some light
    this.pointLight2 = new THREE.PointLight(0xa8a8a8);
    //this.pointLight2 = new THREE.AmbientLight(0xFFFFFF);
    // set its position
    this.pointLight2.position.x = 0; // 10;
    this.pointLight2.position.y = 130; // 50;
    this.pointLight2.position.z = 0; // 130;
    // add to the scene
    ...
    */
	...
	for( var i in this.scene.children ) {
		...
		item.position.x       = Math.cos(angle) * item.metaData.distance * options.distanceFactor;
		item.position.z       = Math.sin(angle) * item.metaData.distance * options.distanceFactor;
...
function buildSolarSystem( parentMesh, solarSystem, textures, origin, options ) {

    if( !options.distanceFactor ) options.distanceFactor = 0.00000003;
    if( !options.sizeFactor     ) options.sizeFactor     = 0.0001;
	...
	var entitySettings = solarSystem.children[k];
	console.debug( "entitySettings=" + JSON.stringify(entitySettings) );
	var radius   = (entitySettings.size/2.0) * options.sizeFactor;
	var geometry = null;
	var material = null;
	var item     = null;
	if( entitySettings.type == "ring" ) {
	    /*
	    geometry = new THREE.TorusGeometry( radius*4.5,  // radius
						radius*1.5,  // tube diameter
						16,          // radialSegments
						32,          // tubularSegment
						Math.PI*2    // arc
					      );
					      */
	    geometry = new IKRS.RingGeometry( radius*1.1, radius*1.8, 256 );
	    // Pick a material, something like MeshBasicMaterial, PhongMaterial, 
	    material = new THREE.MeshBasicMaterial( { map          : textures[entitySettings.id].texture,
						      alphaMap     : textures["saturn.ring.alpha"].texture,
						      emissive     : 0x888888
						      //vertexColors : THREE.FaceColors
						    } ); 	
	    material.transparent = true;    // Set this to activate the alpha mapping
	    // Create the cube from the geometry and the material ...
	    ...
	    // Pick a material, something like MeshBasicMaterial, PhongMaterial, 
	    material = new THREE.MeshLambertMaterial( { map: textures[entitySettings.id].texture } ); 	
	    // Create the cube from the geometry and the material ...
	    item = new THREE.Mesh( geometry, material ); 
	}
	...
	item.position.x += origin.x;
	item.position.y += origin.y;
	item.position.z += origin.z + entitySettings.distance*options.distanceFactor;
	// ... and add it to your scene.
	this.scene.add( item ); 

	// Add text sprite
	if( entitySettings.label ) {
	    var nameSprite = makeTextSprite( " " + entitySettings.label + " ", 
					     { fontsize: 32, fontface: "Courier", borderColor: null } ); //{r:0, g:0, b:255, a:1.0} } );
	    nameSprite.position.set( origin.x, origin.y, origin.z + entitySettings.distance*options.distanceFactor );
	    //this.scene.add( nameSprite );
	    item.children.push( nameSprite );
	    item.nameSprite = nameSprite;
	}
	...
	// Add orbit line
	var orbitGeometry = new IKRS.OrbitGeometry( entitySettings.distance*options.distanceFactor, 100 );
	var lineMaterial  = new THREE.LineBasicMaterial( { color    : 0x0088f8, transparent : true, opacity : 0.5, linewidth : 1.0 });
	var orbit = new THREE.Line( orbitGeometry, lineMaterial );
	//orbit.rotation.x = Math.PI / 2;
	//circle.position.y = cylinderParam.trackHeight+20;
	...
	// Item has children?
	if( entitySettings.children ) {
	    buildSolarSystem( item,           // parentMesh
			      entitySettings, 
			      textures,
